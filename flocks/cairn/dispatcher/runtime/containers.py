...
from flocks.cairn.dispatcher.config import ContainerConfig
...
class ContainerManager:
    ...
    def _ensure_running_locked(self, project_id: str, name: str) -> str:
        ...
        if state is not None:
            ...
            return name
        LOG.info("creating container project=%s container=%s", project_id, name)
        try:
    ...
    def create_startup_container(self) -> str:
        ...
        LOG.debug("creating startup healthcheck container container=%s", name)
        try:
            self._client.containers.run(
                self._config.image,
                ["sleep", "infinity"],
    ...
    def inspect_state(self, name: str) -> str | None:
        ...
        try:
            ...
            raise RuntimeError(f"failed to inspect state of container {name}: {exc}") from exc

    def create(self, name: str, image: str, env: dict[str, str] | None = None) -> str:
        self._require_docker()
        assert self._client is not None
        try:
            ...
            if self._is_name_conflict(exc):
                LOG.warning("container already exists name=%s", name)
    ...
    def get_logs(self, name: str, tail: int = 100) -> str:
        ...
        try:
            ...
            raise RuntimeError(f"failed to get logs for container {name}: {exc}") from exc

    def wait(self, name: str, timeout: int | None = None) -> int:
        self._require_docker()
        assert self._client is not None
        container = self._client.containers.get(name)
    ...
    def start(self, name: str) -> None:
        ...
        try:
            ...
            raise RuntimeError(f"container not found: {name}") from None
        except DockerException as exc:
    ...
    def stop(self, name: str, timeout: int = 10) -> None:
        ...
        try:
            ...
            LOG.warning("container not found for stop name=%s", name)
        except DockerException as exc:
    ...
    def remove(self, name: str, force: bool = True) -> None:
        ...
        try:
            ...
            LOG.warning("container not found for removal name=%s", name)
        except DockerException as exc:
    ...
    def cleanup_completed(self, project_id: str) -> bool:
        ...
        if self._config and self._config.completed_action == "remove":
    ...
    def cleanup_stopped(self, project_id: str) -> bool:
        ...
        try:
            ...
            LOG.warning("failed to stop stopped container=%s error=%s", name, exc)
            return False
        return self.inspect_state(name) != "running"
    ...
    def build_exec_process(
        ...
        from flocks.cairn.dispatcher.runtime.process import ManagedProcess
    ...
    @staticmethod
    def _is_name_conflict(exc: APIError) -> bool:  # type: ignore[var-annotated]
