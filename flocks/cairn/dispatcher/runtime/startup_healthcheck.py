...
from flocks.cairn.dispatcher.tasks.common import run_direct_healthcheck, run_healthcheck
...
def run_startup_healthchecks(
    ...
    *,
    ...
    container_workers = [w for w in workers if not get_driver(w.type).can_execute_direct()]
    direct_workers = [w for w in workers if get_driver(w.type).can_execute_direct()]

    if container_workers and container_manager.disabled:
        LOG.warning(
            "startup healthcheck skipped for container-dependent workers because container manager is disabled workers=%s",
            [w.name for w in container_workers],
        )
        skipped_results = [
            StartupHealthcheckResult(
                ...
                duration_ms=0,
                http_status="skipped",
                response_preview="container manager disabled",
                stderr_preview="container manager disabled",
                command="-",
            )
            for w in container_workers
        ]
    else:
        skipped_results = []

    if not container_workers or container_manager.disabled:
        container_name = None
    else:
        ...
        with ThreadPoolExecutor(max_workers=parallelism) as executor:
            ...
            for worker in direct_workers:
                future_map[executor.submit(
                    _run_worker_healthcheck_direct,
                    worker,
            ...
            if container_name is not None:
                for worker in container_workers:
                    future_map[executor.submit(
                        _run_worker_healthcheck_container,
                        container_manager,
                        container_name,
                        worker,
                        config.runtime.healthcheck_timeout,
            ...
            results: list[StartupHealthcheckResult] = skipped_results.copy()
...
def _run_worker_healthcheck_direct(
    worker: WorkerConfig,
    timeout_seconds: int,
) -> StartupHealthcheckResult:
...
def _run_worker_healthcheck_container(
    container_manager: ContainerManager,
    container_name: str,
    worker: WorkerConfig,
    timeout_seconds: int,
