...
def run_direct_healthcheck(
    ...
    *,
    ...
    duration_ms = int((time.perf_counter() - started) * 1000)
    return HealthcheckRun(result=result.to_process_result(), duration_ms=duration_ms)
...
def write_graph_snapshot_local(graph_yaml: str, *, phase: str) -> str:
    snap_dir = tempfile.mkdtemp(prefix=f"cairn-{phase}-")
    path = os.path.join(snap_dir, "graph.yaml")
...
def record_session_log(
    ...
    *,
    ...
    if result.ok and result.data and fact_ids:
