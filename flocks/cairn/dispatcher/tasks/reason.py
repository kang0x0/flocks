...
from flocks.cairn.dispatcher.contracts import parse_json_output, validate_reason_payload
from flocks.cairn.dispatcher.prompting import (
    format_fact_ids,
    format_open_intents,
    load_prompt,
    render_prompt,
...
from flocks.cairn.models import ProjectDetail
...
def run_reason_task(
    ...
    if driver.can_execute_direct():
        return _run_reason_direct(config, client, driver, project, export_yaml, worker, cancellation)
    return _run_reason_container(config, client, container_manager, driver, project, export_yaml, worker, cancellation)


def _run_reason_direct(
    config: DispatchConfig,
    client: CairnClient,
    driver,
    project: ProjectDetail,
    ...
    try:
        ...
        prompt = render_prompt(
            ...
            {
                "graph_yaml": write_graph_snapshot_local(export_yaml.strip(), phase="reason_execute"),
